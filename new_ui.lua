...
local LucideIcons = loadstring(game:HttpGet("https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/main/icons.lua"))()
...
local function GetLucideIconData(iconName)
    local iconData = LucideIcons["48px"] and LucideIcons["48px"][iconName]
    if iconData then
        return {
            assetId = "rbxassetid://" .. iconData[1],
            rectSize = iconData[2],
            rectOffset = iconData[3]
        }
    end
    return nil
end
...
function MacLib:Window(Settings)
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
    			...
    			-- LUCIDE ICON SUPPORT
    			local tabImage
    			if Settings.LucideIcon then
        			local iconInfo = GetLucideIconData(Settings.LucideIcon)
        			if iconInfo then
            			tabImage = Instance.new("ImageLabel")
            			tabImage.Name = "TabImage"
            			tabImage.Image = iconInfo.assetId
            			tabImage.ImageRectSize = Vector2.new(iconInfo.rectSize[1], iconInfo.rectSize[2])
            			tabImage.ImageRectOffset = Vector2.new(iconInfo.rectOffset[1], iconInfo.rectOffset[2])
    			...
    			tabSwitcherName.FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json", Enum.FontWeight.Medium, Enum.FontStyle.Normal)
